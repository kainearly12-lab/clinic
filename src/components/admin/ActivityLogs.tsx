...
export const ActivityLogs = React.memo(function ActivityLogs({ logs, onRefresh, isLoading, onNotify }: ActivityLogsProps) {
  ...
  const handleExecuteCleanup = async () => {
    ...
    try {
      ...
      if (res.success) {
        ...
        const msg =
          count > 0
            ? `تم بنجاح حذف ${count} من سجلات النشاط التي مر عليها أكثر من 30 يوماً.`
            : 'لا توجد سجلات نشاط قديمة (أكثر من 30 يوماً). جميع السجلات الحالية حديثة.';
        ...
        // Trigger parent toast notification if provided
        if (onNotify) {
          onNotify('success', msg);
        }

        // Refresh UI state without page reload
        await onRefresh();

        // Auto close dialog modal on success after brief pause
        setTimeout(() => {
          setIsCleanupModalOpen(false);
        }, 1200);
      } else {
        const errorMsg = res.error || 'فشلت عملية حذف السجلات القديمة من Supabase';
        setCleanupResult({
          success: false,
          message: errorMsg,
        });
      ...
      const errorMsg = err instanceof Error ? err.message : 'تعذر إتمام عملية التنظيف';
      setCleanupResult({
        success: false,
        message: errorMsg,
      });
...
SELECT cron.schedule(
  ...
  return (
    <div className="space-y-6">
      ...
      {/* ========================================================================= */}
      {/* 🌟 MODAL 1: MANUAL 30-DAY LOGS RETENTION CLEANUP CONFIRMATION MODAL        */}
      {/* ========================================================================= */}
      {isCleanupModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm animate-fade-in">
          <div
            className="relative w-full max-w-md rounded-2xl bg-gradient-to-b from-slate-900 to-slate-950 border border-white/10 shadow-2xl p-6 space-y-5"
            dir="rtl"
          >
            {/* Modal Header */}
            <div className="flex items-center justify-between pb-3 border-b border-white/10">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-base font-bold text-white">تأكيد مسح السجلات القديمة</h3>
                  <p className="text-xs text-slate-400">سياسة الاحتفاظ بسجلات النشاط (30 يوماً)</p>
                </div>
              </div>
              <button
                type="button"
            ...
            <div className="space-y-3 text-xs leading-relaxed text-slate-300">
              <div className="p-3.5 rounded-xl bg-red-500/10 border border-red-500/20 text-red-200 flex items-start gap-3">
                ...
                <p className="text-xs text-red-200/90 leading-relaxed font-medium">
                  هل أنت متأكد؟ سيتم حذف كافة سجلات النشاط التي مر عليها أكثر من 30 يوماً نهائياً.
                </p>
              </div>
            ...
            <div className="flex items-center justify-end gap-2.5 pt-2 border-t border-white/10">
              <button
                ...
                disabled={isCleaning}
                className="px-4 py-2 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-300 text-xs font-bold transition disabled:opacity-50 cursor-pointer"
              >
                إلغاء
              </button>
              ...
              <button
                ...
                disabled={isCleaning}
                className="px-5 py-2 rounded-xl bg-red-600 hover:bg-red-700 text-white text-xs font-bold shadow-lg shadow-red-600/30 transition-all flex items-center gap-2 disabled:opacity-50 cursor-pointer"
              >
                {isCleaning ? (
                  <>
